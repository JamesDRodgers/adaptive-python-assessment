...
async function start() {
    try {
        ...
        showError("Failed to start assessment. Please check your connection and refresh the page.", error);
    }
}
...
async function submitAnswer() {
    ...
    try {
        ...
        // Wait minimum 2 seconds for user to read feedback
        ...
        if (error.message.includes("Session not found")) {
            showError("Your session has expired. Please start a new assessment.", error);
        } else if (error.message.includes("Assessment already completed")) {
            showError("This assessment is already complete. Please start a new one.", error);
        } else if (error.message.includes("No active question")) {
            showError("Session error occurred. Please start a new assessment.", error);
        } else {
            showError("Failed to submit answer. Please check your connection and try again.", error);
        }
    }
}
